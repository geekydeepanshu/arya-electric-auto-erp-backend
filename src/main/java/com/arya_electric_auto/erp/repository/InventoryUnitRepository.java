...
public interface InventoryUnitRepository extends JpaRepository<InventoryUnit, Long> {
	...
	@Query("SELECT COUNT(i) FROM InventoryUnit i WHERE i.product.id = :productId AND i.status = 'IN_STOCK'")
	Long countAvailableByProduct(@Param("productId") Long productId);

    boolean existsBySerialNumber(String serialNumber);
    
    List<InventoryUnit> findByStatus(InventoryStatus status);

    List<InventoryUnit> findByProductIdAndStatus(Long productId, InventoryStatus status);
}